...
edited directly.
    ...
    python3 resource/helper/make_scenes.py
...
def scene2_style(used):
    return f"""        <style>
            /* ── Mapping only: class -> variable. The VALUES live in
               ...
               file opened on its own shows paper rather than unstyled shapes. ⚠️ GENERATED — see resource/helper/make_scenes.py. */
...
def scene3_style(used):
    return f"""        <style>
            /* ── Mapping only: class -> variable. The VALUES live in
               ...
               — see resource/helper/make_scenes.py. */
